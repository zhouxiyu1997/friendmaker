...
**双重根因**：
1. `sendReport()` 中 HAT 位偏移与描述符不一致（第一层问题，§3.5 已修复）
2. **`init()` 中 `rpt_` 预填缓冲区整体偏移 1 字节**（第二层问题，本次修复）

`init()` 旧代码将 HAT 值写入 `rpt_[1]`（按钮高 6 位区域），摇杆值从 `rpt_[2]` 开始填充，导致 `rpt_[6]`（Rz 轴 = 右摇杆 Y）被填为 **0** 而非归中值 **128**。Switch 在 USB 枚举后立即采样首个 HID 报告进行摇杆校准，0 被记录为中点后，所有后续 128 归中报告均被解释为向下偏移。这就是即使 `sendReport()` 打包逻辑已修复、漂移仍然持续的原因。

**修复方案**：
- `sendReport()` 打包逻辑已在 §3.5 修复
- `init()` 预填缓冲区偏移修复（commit `a40a9a1`）：`rpt_[1]=0`, `rpt_[2]=HAT_CENTER`, `rpt_[6]=STICK_CENTER`
- 烧录验证通过：TCP 左右摇杆全方向测试（8 方向 + 归中）均正常
...
1. ✅ **验证右摇杆修复** — 重新编译烧录，确认报告格式对齐后右摇杆不再漂移。init() 预填缓冲区偏移修复 (commit `a40a9a1`)，TCP 全方向测试通过
