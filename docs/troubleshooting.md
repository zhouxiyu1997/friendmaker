...
如果按 `EN` 重启后现象就消失，通常更接近开发板当前状态、蓝牙 HID 报告链路状态、供电或温度波动被刷新；这时更建议先重新做一轮 `手柄测试` 和 `调试测速`，不要直接用上一轮异常绘制结果判断颜色识别或 `官方色绘制` 本身是否有问题。
...
- 是否已经先跑过 `调试测速`
- `脚本生成` 页里当前选择的画笔是否就是你要执行的那一档；正式绘制会自动切到这里选中的笔刷
- 光标是否从 `画布中心` 起步
- 开始绘制后是否还手动碰过手柄或屏幕
...
当前版本已经尽量让网页预览、命令用色和最终绘制用色保持一致；如果你仍看到轻微差异，通常更接近实机显示和画笔表现的影响，而不是网页预览单独选错了颜色。

常见原因：

- 实机观感会受到画笔大小、边缘抗锯齿和游戏内显示效果影响
- `官方色绘制` 仍然依赖游戏内置 `7x12` 色盘，不可能和任意原图逐像素完全一致
- 如果你手动改过右侧 `9` 个色盘槽位，`官方色绘制` 的前提会被破坏

建议：

1. 先确认当前模式、色数和下方“当前预览用色”与预期一致
2. 使用 `官方色绘制` 时，保持游戏默认的 `9` 个色盘槽位颜色
3. 对颜色特别敏感的图，优先尝试更高的色数和更小的画笔
4. 如果是第一次试用，先用结构简单的图片验证整体链路
