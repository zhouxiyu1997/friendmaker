...
function syncStudioUi() {
  ...
  els.executionHint.textContent =
    state.studio.colorMode === "mono"
      ? `当前会把按 ${generatedProfile.imageScalePercent}% 缩放、${describeImagePosition(generatedProfile.imageOffsetXPercent, generatedProfile.imageOffsetYPercent, false)}后的 256x256 黑白脚本通过串口发送到 ${state.selectedPortPath}，模板为“${generatedProfile.templateLabel}”。开始后 ESP32 会先按 X、X 打开笔刷页，并在切笔刷阶段临时切到更保守的输入节奏，从默认的 7 像素圆点笔刷自动切到 ${generatedProfile.brushSize} 像素${generatedProfile.brushShape === "round" ? "圆形像素笔刷" : "方块像素笔刷"}；连按三次 A 完成选中和返回画布后，会恢复当前 timing，再额外等待约 3 秒，最后从画布中心继续翻译成方向键移动与 A 绘制。`
      : generatedProfile.colorMode === "official"
        ? `当前会把按 ${generatedProfile.imageScalePercent}% 缩放、${describeImagePosition(generatedProfile.imageOffsetXPercent, generatedProfile.imageOffsetYPercent, false)}后的 256x256 官方色脚本通过串口发送到 ${state.selectedPortPath}，模板为“${generatedProfile.templateLabel}”。请先保持右侧 9 个槽位默认颜色不变；开始后 ESP32 会先按 X、X 打开笔刷页，并在切笔刷阶段临时切到更保守的输入节奏，从默认的 7 像素圆点笔刷自动切到 ${generatedProfile.brushSize} 像素${generatedProfile.brushShape === "round" ? "圆形像素笔刷" : "方块像素笔刷"}；连按三次 A 完成选中和返回画布后，会恢复当前 timing，再额外等待约 3 秒，随后按这组默认槽位状态去配置内置 7x12 色盘并继续绘制。`
        : `当前会把按 ${generatedProfile.imageScalePercent}% 缩放、${describeImagePosition(generatedProfile.imageOffsetXPercent, generatedProfile.imageOffsetYPercent, false)}后的 256x256 自动量化多色脚本通过串口发送到 ${state.selectedPortPath}，模板为“${generatedProfile.templateLabel}”。开始后 ESP32 也会先按 X、X 打开笔刷页，并在切笔刷阶段临时切到更保守的输入节奏，从默认的 7 像素圆点笔刷自动切到 ${generatedProfile.brushSize} 像素${generatedProfile.brushShape === "round" ? "圆形像素笔刷" : "方块像素笔刷"}；连按三次 A 完成选中和返回画布后，会恢复当前 timing，再额外等待约 3 秒，随后分批把用于补偿的自定义色槽颜色写入 9 个槽位后再绘制。当前主预览显示的是目标 / 成品预计色，不一定等于发送到色槽里的更深补偿色。`;
  renderStudioConnectionStatus();
  syncDesktopShellUi();
}
